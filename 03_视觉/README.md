...
**出图前必过设计规范门禁**：读取 [`Shopee设计系统/L1_原子变量`](Shopee设计系统/L1_原子变量/README.md) 与 Figma `✅ Design Token 设计令牌` 页（`Seller Center Library` node `1:2`），再按需读取组件层与页面层。Token 层读取按「Token 使用规范」变更检测执行：同一次连续执行只读一次，跨反馈轮次先检查未变化则复用、变化才重读；不允许凭记忆、不允许降级。组件与页面文档可能落后于 library，按 L5「未建组件降级规则」处理并登记。完整定义见 [`设计规范门禁`](../00_总流程/Design%20Loop总流程与调用时机.md)。
...
视觉阶段只执行已选定的产品与交互方向，不改变上游目标和核心链路，也不改动 Step 1 已确认的产品逻辑。出稿必须覆盖产品逻辑中的触发前、触发后与撤下后状态。
